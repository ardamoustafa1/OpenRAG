...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("settings", postgresql.JSONB(), nullable=False, server_default='{"chunk_size": 1000, "overlap": 200}'),
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("uuid_generate_v4()")),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("collection_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("document_collections.id", ondelete="CASCADE"), nullable=False, index=True),
    ...
    op.create_table(
        ...
        sa.Column("qdrant_point_id", postgresql.UUID(as_uuid=True), nullable=False, index=True),
        sa.Column("metadata", postgresql.JSONB(), nullable=False, server_default="{}"),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
    )
    ...
    op.create_table(
        ...
        sa.Column("is_active", sa.Boolean(), nullable=False, server_default="true"),
        sa.Column("token_quota_monthly", sa.BigInteger(), nullable=False, server_default="1000000"),
    ...
    # -------------------------------------------------------------------------
    for table in ["document_collections", "documents", "document_chunks", "conversations", "messages", "api_keys"]:
