...
class PromptGuard:
    ...
    JAILBREAK_PATTERNS = [
        ...
        re.compile(r"you\s+are\s+(now\s+)?a\s+(developer|unrestricted|DAN)", re.IGNORECASE),
    ...
    def scan_user_input(cls, text: str) -> ScanResult:
        ...
        for pattern in cls.JAILBREAK_PATTERNS:
            if pattern.search(text):
                ...
                return ScanResult(is_safe=False, reason="Malicious prompt injection pattern detected.")
    ...
    def sanitize_output(cls, text: str) -> str:
        ...
        sanitized = re.sub(r"(AI_PLATFORM_INTERNAL_ID_[A-Z0-9]+)", "***REDACTED***", sanitized)
