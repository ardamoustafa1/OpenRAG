...
class ChunkingService:
        ...
        for el in elements:
            ...
            if el_type == "Title":
                ...
                if current_chunk_text:
                    chunks.extend(self._flush_chunk(current_chunk_text, current_section_title, document_id))
                    current_chunk_text = []
                    current_chunk_tokens = 0
            ...
            if el_type == "Table":
                ...
                if tokens > self.max_tokens * 2:
                    ...
                    chunks.extend(self._recursive_split(text, current_section_title, document_id))
                else:
                    chunks.append(self._build_chunk_dict(text, tokens, current_section_title, document_id))
                continue
    ...
    def _flush_chunk(self, chunk_texts: list[str], section_title: str, document_id: str) -> list[dict]:
        joined_text = "\n\n".join(chunk_texts)
    ...
    def _recursive_split(self, text: str, section_title: str, document_id: str) -> list[dict]:
        ...
        for p in paragraphs:
            if token_counter.count_tokens(curr_text + "\n\n" + p) > self.max_tokens:
                if curr_text:
                    chunks.append(self._build_chunk_dict(curr_text, token_counter.count_tokens(curr_text), section_title, document_id))
                curr_text = p
            else:
    ...
    def _build_chunk_dict(self, text: str, tokens: int, section_title: str, document_id: str) -> dict:
        return {
