...
class VectorStoreService:
    ...
    async def ensure_collection(self, tenant_id: str, collection_id: str, vector_size: int = 1024):
    ...
    async def upsert_chunks(self, tenant_id: str, collection_id: str, chunks: list[dict]):
        ...
        try:
            ...
            logger.info("Upserted chunks to Qdrant", count=len(points), collection_name=col_name)
        except Exception as e:
            logger.error("Failed to upsert points to Qdrant", error=str(e))
            raise

    async def delete_by_document(self, tenant_id: str, collection_id: str, document_id: str):
