...
if settings.STRIPE_API_KEY:
    ...
    logger.warning(
        "STRIPE_API_KEY is not configured. Billing features will be unavailable."
    )
...
async def create_stripe_portal(
    ...
    stmt = (
        select(TenantSubscription.stripe_customer_id)
    ...
    if not stripe_customer_id:
        raise HTTPException(
            status_code=400, detail="Tenant has no linked Stripe customer account"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Billing service unavailable"
        ) from None
...
async def stripe_webhook(
    ...
    try:
        ...
        if event_type == "customer.subscription.deleted":
            ...
            if lines and "period" in lines[0]:
                period_end = datetime.fromtimestamp(
                    lines[0]["period"]["end"], tz=timezone.utc
                )
                stmt = (
                    update(TenantSubscription)
                    .where(TenantSubscription.stripe_customer_id == customer_id)
        ...
        await db.rollback()
        raise HTTPException(
            status_code=500, detail="Webhook processing error"
        ) from None
