...
async def delete_tenant(
    ...
    return {"message": f"Tenant {'hard' if hard_delete else 'soft'} deleted successfully"}
...
async def impersonate_tenant(
    ...
    if not tenant_admin:
        raise HTTPException(status_code=404, detail="No tenant admin found for this tenant")
        
    token = create_access_token(subject=str(tenant_admin.id))
    return {"access_token": token, "token_type": "bearer", "impersonated_user": tenant_admin.email}
