...
async def send_message(
    ...
    async def sse_generator():
        ...
        try:
            # A. Retrieve
            retrieved = await retriever.retrieve(query=payload.content, tenant_id=str(tenant.id), collection_id=payload.collection_id)
            
            # B. Re-rank
            reranked = reranker.rerank(query=payload.content, retrieved_chunks=retrieved, top_k=10)
            
            # C. Context Build
            used_chunks, context_str = context_builder.build_context(reranked, max_context_tokens=4000)
            
            # D. Format Sources
            sources = citation_service.format_sources(used_chunks)
            
            # E. Stream Generation
            # Let's assume we use the primary model "qwen2.5-72b"
            model_to_use = tenant.settings.get("default_model", "qwen2.5-72b")
            
            # We don't save the assistant message to DB immediately during stream
            # In a real app, you accumulate the chunks and save at the end, 
            # or do it via background task. For simplicity, we just stream it here.
            full_response = ""
