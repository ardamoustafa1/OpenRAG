...
def send_webhook(self, webhook_id: str, event_type: str, payload: dict):
    ...
    try:
        response = httpx.post(mock_url, json=full_payload, headers=headers, timeout=10.0)
        
        if 200 <= response.status_code < 300:
            logger.info("Webhook delivered successfully", webhook_id=webhook_id, status=response.status_code)
            return "Success"
        elif 400 <= response.status_code < 500:
            logger.error("Webhook delivery failed (Client Error). Won't retry.", 
                         webhook_id=webhook_id, status=response.status_code, response=response.text)
            return "Failed - No Retry"
        else:
            logger.warning("Webhook delivery failed (Server Error). Retrying...", 
                           webhook_id=webhook_id, status=response.status_code)
            raise self.retry(exc=Exception(f"Server returned {response.status_code}"))
            
    except httpx.RequestError as exc:
        logger.warning("Webhook delivery network error. Retrying...", webhook_id=webhook_id, error=str(exc))
