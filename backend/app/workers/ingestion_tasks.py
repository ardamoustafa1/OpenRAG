...
async def emit_progress(
    ...
    logger.info(
        "Progress updated", document_id=document_id, stage=stage, progress=progress
    )
...
async def _process_document_async(
    ...
    async with async_session_factory() as db:
        try:
            await emit_progress(
                redis, document_id, "extraction", 0.1, "Downloading document..."
            )
            ...
            await emit_progress(
                redis, document_id, "extraction", 0.3, "Extracting text and metadata..."
            )
            extraction_result = extraction_service.process_file(
            ...
            await emit_progress(
                redis, document_id, "chunking", 0.5, "Chunking document structurally..."
            )
            ...
            # Step 6: Embedding
            await emit_progress(
                redis, document_id, "embedding", 0.7, "Generating vector embeddings..."
            )
            ...
            await emit_progress(
                redis, document_id, "indexing", 0.9, "Indexing vectors to Qdrant..."
            )
            ...
            await emit_progress(
                redis, document_id, "completed", 1.0, "Document processed successfully."
            )
            logger.info(
                "Document processing completed successfully", document_id=document_id
            )
...
def process_document(
    ...
    try:
        ...
        logger.error(
            "Task failed, scheduling retry", document_id=document_id, error=str(exc)
        )
...
async def _build_bm25_index_async(tenant_id: str, collection_id: str) -> None:
    ...
    async with async_session_factory() as db:
        try:
            ...
            mapping = [
                {"id": str(chunk.qdrant_point_id), "payload": chunk.metadata_}
                for chunk in chunks
            ]
            ...
            logger.info(
                "BM25 index built and cached successfully",
            ...
            logger.error(
                "BM25 index build failed", error=str(e), collection_id=collection_id
            )
        finally:
            await redis.close()
