...
current_tenant_id_var: ContextVar[str | None] = ContextVar("current_tenant_id", default=None)
...
async def get_db_session(request: Request) -> AsyncGenerator[AsyncSession, None]:
    ...
    async with async_session_factory() as session:
        ...
        try:
            yield session
            # We don't automatically commit here, users should session.commit() manually
            # But we could do it if we want transaction-per-request paradigm.
        finally:
