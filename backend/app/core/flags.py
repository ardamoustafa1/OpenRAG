...
class FeatureFlags:
    ...
    async def is_enabled(self, feature_name: str, tenant_id: UUID | str = None, default: bool = False) -> bool:
        """
        Check if a feature is enabled. 
        Checks tenant-specific flag first, then falls back to global flag.
        """
        if not self.redis:
            logger.warning("Feature flags checking without Redis initialized. Returning default.")
            return default

        try:
            # 1. Check tenant override
            if tenant_id:
                tenant_flag = await self.redis.get(f"ff:tenant:{tenant_id}:{feature_name}")
    ...
    async def set_tenant_flag(self, feature_name: str, tenant_id: UUID | str, value: bool):
        if self.redis:
            await self.redis.set(f"ff:tenant:{tenant_id}:{feature_name}", json.dumps(value))
