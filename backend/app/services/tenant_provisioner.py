...
class TenantProvisioner:
    ...
    async def provision_tenant(self, db: AsyncSession, name: str, slug: str, admin_email: str, plan_name: str = "starter") -> Tenant:
        """
        ...
        try:
            ...
            logger.error("Error during infrastructure provisioning", tenant_id=str(tenant.id), error=str(e))
            # In a real app, you might want to rollback the DB or queue a retry.
            raise

    async def deprovision_tenant(self, db: AsyncSession, tenant_id: uuid.UUID, hard_delete: bool = False):
        """
        ...
        """
        logger.warning("Deprovisioning tenant", tenant_id=str(tenant_id), hard_delete=hard_delete)
