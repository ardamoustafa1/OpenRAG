...
async def setup_document_test(db_session: AsyncSession):
    ...
    user = User(id=uuid.uuid4(), tenant_id=tenant.id, email="doc@test.com", name="Doc User")
    ...
    return {"tenant": tenant, "user": user, "collection": col, "headers": headers, "db": db_session}

@pytest.mark.asyncio
@patch("app.api.v1.documents.process_document.delay")
@patch("app.api.v1.documents.storage_service.upload_file")
async def test_document_upload_queues_celery_task(mock_upload, mock_delay, client: AsyncClient, setup_document_test):
    """
    ...
    response = await client.post(
        f"/api/v1/collections/{col_id}/documents/upload", 
        files=files, 
        headers=headers
    )
    ...
    doc = (await db.execute(select(Document).where(Document.id == uuid.UUID(data["document_id"])))).scalars().first()
    assert doc is not None
    assert doc.status == "processing"

@pytest.mark.asyncio
@patch("app.api.v1.documents.process_document.delay")
async def test_document_retry_failed(mock_delay, client: AsyncClient, setup_document_test):
    """
    ...
    response = await client.post(f"/api/v1/collections/{col_id}/documents/{doc.id}/retry", headers=headers)
